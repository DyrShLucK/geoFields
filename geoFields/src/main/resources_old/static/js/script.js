...
async function updateNDVI() {
    ...
    statusBox.innerText = `⏳ Запрос к Python (GEE)... Поле: ${selectedFieldDbId}`;

    try {
        // Стучимся напрямую в Питон на порт 8001
        const url = `${PYTHON_SERVICE_URL}/get_ndvi_by_id?field_id=${encodeURIComponent(selectedFieldDbId)}&date=${encodeURIComponent(date)}`;

        console.log("Запрос к Python-сервису:", url);

        const response = await fetch(url, { method: 'GET' });

        if (!response.ok) {
            const errData = await response.json();
            alert(`Ошибка микросервиса: ${errData.detail || response.status}`);
            statusBox.innerText = "❌ Ошибка расчета";
            return;
        }

        const data = await response.json();
        if (!data.url) throw new Error("Микросервис не вернул URL тайлов");

        // Отображаем на карте
        addNdviToMap(data.url, `NDVI поля ${selectedFieldDbId}`);
        statusBox.innerText = "✅ NDVI получен от Python";

    } catch (err) {
        console.error(err);
        alert(err.message || "Ошибка связи с микросервисом");
        statusBox.innerText = "❌ Ошибка соединения с портом 8001";
    }
}
...
function addNdviToMap(url, label) {
    // 1. Генерируем уникальный ID для этого сеанса
    const sourceId = 'ndvi-source';
    const layerId = 'ndvi-layer';

    // 2. Полностью удаляем старое, если оно есть
    if (map.getLayer(layerId)) map.removeLayer(layerId);
    if (map.getSource(sourceId)) map.removeSource(sourceId);

    // 3. Добавляем "анти-кэш" к URL (чтобы браузер не подсовывал старые ошибки)
    const cleanUrl = url + "?t=" + new Date().getTime();

    console.log("MapLibre запрашивает тайлы по адресу:", cleanUrl);
    ...
    // 4. Передвигаем камеру к полю, чтобы точно его увидеть
    // Это поможет, если ты смотришь не в ту часть карты
    const bounds = new maplibregl.LngLatBounds();
    // Мы берем границы из загруженных полей
    const feature = loadedFieldsGeoJson.features.find(f => f.properties.id == selectedFieldDbId);
    if (feature) {
