...
ПРАВИЛА:
1. name: Название поле за неимением выбери колонку с БОЛЬШИМ unique (ID, кадастр, код). НЕ название региона.
2. area: Числовая площадь.
3. active: 1 - по умолчанию, если поле вышло из эксплуатации: 0.
4. history.*: Урожай, даты, культура (поле cropName - только название культуры, без других данных, чисел, иначе верни null).
5. cropYear: Если есть отдельная колонка года → укажи её. Проверь год на коректность (1900-2099). Если нет → верни null.
